...
PublicOrderRateLimitSchema.index({ storeId: 1, phoneHash: 1, windowStartedAt: 1 }, { unique: true })
PublicOrderRateLimitSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0 })

export default mongoose.model<IPublicOrderRateLimit>('PublicOrderRateLimit', PublicOrderRateLimitSchema)
