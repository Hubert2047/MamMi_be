...
export interface IGuestCart extends Document {
  ...
  source: GuestCartSource;
  type: GuestCartType;
  table: string;
  tableSessionId?: mongoose.Types.ObjectId;
  customer?: GuestCartCustomer;
  lines: GuestCartLine[];
  status: "draft" | "confirming" | "confirmed";
  orderId?: mongoose.Types.ObjectId;
  expiresAt: Date;
}
const GuestCartLineSchema = new Schema<GuestCartLine>(
  {
    itemId: { type: String, required: true },
    quantity: { type: Number, required: true },
    variant: String,
    optionSelections: [{ groupId: String, optionId: String, _id: false }],
    noteOptions: { type: [String], default: [] },
...
const GuestCartSchema = new Schema<IGuestCart>(
  {
    ...
    lines: { type: [GuestCartLineSchema], default: [] },
    status: {
      type: String,
      enum: ["draft", "confirming", "confirmed"],
      default: "draft",
    },
