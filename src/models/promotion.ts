...
const PromotionRewardSchema = new Schema(
  {
    ...
    amount: {
      ...
      min: 0,
      validate: {
        validator: function (this: { type: string }, amount: number) {
          return this.type === "percent"
            ? Number.isFinite(amount) && amount <= 100
            : Number.isSafeInteger(amount);
        },
        message: "Promotion money must be a non-negative integer",
      },
    },
  },
  { _id: false },
...
const PromotionSchema = new Schema<IPromotion>(
  {
    ...
    rules: {
      ...
      validate: [
        (rules: unknown[]) => rules.length > 0,
        "A promotion needs at least one rule",
      ],
    },
    status: {
      type: String,
