...
try {
  ...
  if (duplicates.length) {
    throw new Error(
      `Cannot create unique LINE group name index because duplicate names exist: ${JSON.stringify(duplicates)}`,
    );
  }
  ...
  for (const index of indexes) {
    const isNameIndex =
      index.key && Object.keys(index.key).length === 1 && index.key.name === 1;
  ...
  console.log(
    "Migrated LINE groups: removed legacy fields and created unique name index",
  );
} finally {
