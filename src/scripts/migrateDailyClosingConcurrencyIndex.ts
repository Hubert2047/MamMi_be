...
const duplicates = await DailyClosing.aggregate([
    ...
    { $group: { _id: { storeId: '$storeId', periodStart: '$periodStart' }, count: { $sum: 1 }, closingIds: { $push: '$_id' } } },
    { $match: { count: { $gt: 1 } } },
])
if (duplicates.length) {
    await mongoose.disconnect()
    throw new Error(`Cannot create closing concurrency index: ${duplicates.length} confirmed closing period(s) are duplicated. Void the duplicate records first: ${JSON.stringify(duplicates)}`)
}
