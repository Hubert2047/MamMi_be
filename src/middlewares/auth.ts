...
export default async function authenticateToken(
  ...
  if (token == null) {
    ...
    if (!device)
      return res
        .status(401)
        .json({ error: true, message: "Invalid device session" });
    const requestedStoreId = req.headers["x-store-id"];
    if (requestedStoreId && String(requestedStoreId) !== String(device.storeId))
      return res
        .status(403)
        .json({ error: true, message: "You do not have access to this store" });
    req.user = {
  ...
  try {
    ...
    if (requestedStoreId) {
      ...
      if (user.role !== Role.SuperAdmin) {
        const account = await User.findOne({ account: user.account })
          .select({ storeIds: 1 })
          .lean();
