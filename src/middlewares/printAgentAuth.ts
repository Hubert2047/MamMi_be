...
export default function authenticatePrintAgent(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  ...
  if (!token || !agentId)
    return res
      .status(401)
      .json({ success: false, message: "Invalid print agent credentials" });
  void PrintAgent.findOne({ agentId, active: true })
    .select("+tokenHash")
    .lean()
    .then((agent) => {
      const tokenHash = createHash("sha256").update(token).digest("hex");
      if (!agent || !sameSecret(tokenHash, agent.tokenHash))
        return res
          .status(401)
          .json({ success: false, message: "Invalid print agent credentials" });
      void PrintAgent.updateOne(
        { _id: agent._id },
        { $set: { lastSeenAt: new Date() } },
      );
      (req as PrintAgentRequest).printAgent = {
        agentId,
        agentDbId: String(agent._id),
