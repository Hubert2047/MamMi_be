...
describe("realtime channel policy", () => {
  it("routes events to the correct business channel", () => {
    expect(channelForRealtimeEvent("catalog.store-item.price.updated")).toBe(
      "catalog",
    );
  ...
  it("limits each client type to the data it needs", () => {
    expect(channelsForClient("pos", Role.Employee)).toEqual([
      "catalog",
      "orders",
      "finance",
    ]);
    expect(channelsForClient("admin", Role.Admin)).toEqual([
      "catalog",
      "orders",
      "closing",
      "finance",
  ...
  it("always grants superadmin closing visibility even when the client identifies as POS", () => {
    expect(channelsForClient("pos", Role.SuperAdmin)).toEqual([
      "catalog",
      "orders",
      "closing",
      "finance",
  ...
  it("keeps store and order rooms deterministic and isolated", () => {
    expect(roomForStoreChannel("store-a", "orders")).toBe(
      "store:store-a:orders",
    );
    expect(roomForStoreChannel("store-b", "orders")).not.toBe(
      roomForStoreChannel("store-a", "orders"),
    );
