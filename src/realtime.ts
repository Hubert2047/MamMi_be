...
const channelForEvent: Record<string, RealtimeChannel> = {
  ...
  "catalog.promotion.updated": "catalog",
...
export const roomForStoreChannel = (
  storeId: string,
  channel: RealtimeChannel,
) => `${roomForStore(storeId)}:${channel}`;
export const roomForOrder = (orderId: string) => `order:${orderId}`;

export const channelsForClient = (
  clientType: RealtimeClientType,
  role: Role,
): RealtimeChannel[] => {
  if (clientType === "admin" || role === Role.SuperAdmin || role === Role.Admin)
    return ["catalog", "orders", "closing", "finance"];
  if (clientType === "customer") return ["catalog"];
  return ["catalog", "orders", "finance"];
};

export const channelForRealtimeEvent = (
  event: string,
): RealtimeChannel | null => channelForEvent[event] ?? null;
...
const authenticateSocket = async (socket: Socket) => {
  ...
  if (typeof publicToken === "string") {
    ...
    const payload = jwt.verify(publicToken, secret) as {
      scope?: string;
      storeId?: string;
    };
    ...
    const store = await Store.findOne({ _id: storeId, active: true })
      .select({ _id: 1 })
      .lean();
    if (payload.scope !== "public-catalog" || !store)
      throw new Error("Public catalog access denied");
    return {
      storeId,
      publicCatalogOnly: true,
    } satisfies PublicCatalogSocketUser;
  }
  const token = socket.handshake.auth?.token;
  if (typeof token !== "string") {
    const cookieHeader = socket.handshake.headers.cookie || "";
    const deviceToken = cookieHeader.match(
      /(?:^|;\s*)pos_device_session=([^;]+)/,
    )?.[1];
    if (!deviceToken) throw new Error("Missing access token");
    const tokenHash = createHash("sha256")
      .update(decodeURIComponent(deviceToken))
      .digest("hex");
    ...
    return {
      account: `device:${device._id}`,
      role: Role.Employee,
      storeId: String(device.storeId),
    } satisfies StaffSocketUser;
  }
  const payload = jwt.verify(
    token,
    process.env.ACCESS_TOKEN_PRIVATE_KEY as string,
  ) as { account: string; role: Role; storeId: string };
  const storeId = String(
    socket.handshake.auth?.storeId || payload.storeId || "",
  );
  if (
    !storeId ||
    !(await canAccessStore(payload.account, payload.role, storeId))
  )
...
const clientTypeOf = (socket: Socket): RealtimeClientType => {
  ...
  return value === "admin" || value === "customer" || value === "order"
    ? value
    : "pos";
};
...
const leaveStoreRooms = (socket: Socket, storeId: string) => {
  ...
  for (const channel of [
    ...
    socket.leave(roomForStoreChannel(storeId, channel));
};
...
const joinStoreRooms = (socket: Socket, storeId: string) => {
  ...
  for (const channel of channelsForClient(clientType, user.role))
    socket.join(roomForStoreChannel(storeId, channel));
};
...
export const emitStoreEvent = (
  ...
  if (channel === "orders" && typeof payload.orderId === "string")
    io?.to(roomForOrder(payload.orderId)).emit(event, message);
};
...
export const emitCatalogEventToStores = async (
  event: string,
  payload: Record<string, unknown> = {},
  storeIds?: string[],
) => {
  ...
  const ids =
    storeIds ??
    (await Store.find({ active: true }).select({ _id: 1 }).lean()).map(
