...
describe("promotion calculations", () => {
  it("does not make an active promotion available after endsAt, and respects the exact end instant", () => {
    ...
    expect(isPromotionAvailableAt({ status: "active", endsAt }, endsAt)).toBe(
      true,
    );
    expect(
      isPromotionAvailableAt(
  ...
  it("does not make a promotion available before startsAt or when status is not active", () => {
    ...
    expect(
      isPromotionAvailableAt(
        ...
        new Date("2026-08-26T09:59:59.999Z"),
      ),
    ).toBe(false);
    expect(
      isPromotionAvailableAt({ status: "draft", startsAt }, startsAt),
    ).toBe(false);
    expect(isPromotionAvailableAt({ status: "archived" }, new Date())).toBe(
      false,
    );
  });
  ...
  it("rejects a stale client pricing snapshot when amount or promotion version changes", () => {
      ...
      matchesExpectedPromotionPricing(
        {
          ...actual,
          appliedPromotions: [
            { ...actual.appliedPromotions[0]!, promotionVersion: 1 },
          ],
        },
        actual,
      ),
    ...
    expect(
      matchesExpectedPromotionPricing({ ...actual, total: 171 }, actual),
    ).toBe(false);
  ...
  it("clamps every excessive fixed or percentage reward at the remaining amount so the payable total never becomes negative", () => {
    const result = calculatePromotionPricing(
      [
        {
          id: "tea",
          basePrice: 60,
          quantity: 1,
          addons: [{ id: "boba", amount: 1, priceExtra: 20 }],
        },
      ],
      [
        {
          ...
          rules: [
            {
              target: "product",
              productIds: ["tea"],
              reward: { type: "value", amount: 999 },
            },
            {
              target: "addon",
              addonIds: ["boba"],
              reward: { type: "percent", amount: 100 },
            },
        ...
        rules: [
          {
            ...
            reward: { type: "value" as const, amount: 50 },
          },
        ],
      },
    ];
  ...
  it("applies product, addon, line, then order rules on the remaining amounts and records the allocation", () => {
    ...
    expect(result.appliedPromotions[0]?.targets).toEqual([
      "product",
      "addon",
      "line",
      "order",
    ]);
    expect(result.appliedPromotions[0]?.allocations).toEqual([
      {
        itemId: "tea",
        productDiscountAmount: 40,
        addonDiscounts: [{ addonId: "boba", discountAmount: 5 }],
      },
    ]);
  });
  ...
  it("applies every item reward before an order reward from a mixed promotion", () => {
    ...
    expect(
      result.appliedPromotions.map((promotion) => [
      ...
      rules: [
        {
          ...
          reward: { type: "percent" as const, amount: 10 },
        },
      ],
    };
    const single = calculatePromotionPricing(
      [{ id: "tea", basePrice: 95, quantity: 1, addons: [] }],
      [promotion],
    );
    const split = calculatePromotionPricing(
      [
        { id: "tea", basePrice: 95, quantity: 1, addons: [] },
        { id: "coffee", basePrice: 95, quantity: 1, addons: [] },
      ],
      [promotion],
    );
  ...
  it("keeps non-combinable promotions exclusive by priority", () => {
    ...
    expect(
      result.appliedPromotions.map((promotion) => promotion.promotionId),
    ).toEqual(["high"]);
  });
