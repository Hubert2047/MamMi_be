...
describe('promotion calculations', () => {
    ...
    it('requires explicit selection for manual promotions and applies order reductions after item reductions', () => {
        const promotions = [
            { id: 'manual', name: 'Order reward', names: { vi: 'Giảm đơn', en: 'Order reward', 'zh-TW': '整筆折扣' }, version: 1, mode: 'manual' as const, minSubtotal: 100, priority: 1, combinable: true, rules: [{ target: 'order' as const, reward: { type: 'value' as const, amount: 50 } }] },
        ]
        ...
        expect(result.appliedPromotions[0]?.names).toEqual({ vi: 'Giảm đơn', en: 'Order reward', 'zh-TW': '整筆折扣' })
    })
