import { calculateOrderItemTotal, type OrderCalculationItem } from './orderCalculations.js'
...
export type PromotionRule = { target: PromotionTarget; productIds?: string[]; addonIds?: string[]; reward: Reward }
export type PricePromotion = {
    id: string
    name: string
    /** Immutable localized labels retained with the applied promotion snapshot. */
...
export type PromotionOrderItem = Omit<OrderCalculationItem, 'addons'> & { id: string; addons: (OrderCalculationItem['addons'][number] & { id: string })[] }
...
export type PromotionPricing = { total: number; appliedPromotions: AppliedPromotion[] }
export type ExpectedPromotionPricing = Pick<PromotionPricing, 'total' | 'appliedPromotions'>

export const matchesExpectedPromotionPricing = (expected: ExpectedPromotionPricing | undefined, actual: PromotionPricing): boolean => {
    if (!expected || !Number.isFinite(expected.total) || Math.abs(expected.total - actual.total) > 0.0001) return false
    const expectedPromotions = [...expected.appliedPromotions].sort((a, b) => a.promotionId.localeCompare(b.promotionId))
    const actualPromotions = [...actual.appliedPromotions].sort((a, b) => a.promotionId.localeCompare(b.promotionId))
    return expectedPromotions.length === actualPromotions.length && expectedPromotions.every((promotion, index) => {
        const other = actualPromotions[index]
        return other && promotion.promotionId === other.promotionId && promotion.promotionVersion === other.promotionVersion && Math.abs(promotion.discountAmount - other.discountAmount) <= 0.0001
    })
}

export const isPromotionAvailableAt = (promotion: { status: string; startsAt?: Date | string | null; endsAt?: Date | string | null }, now = new Date()): boolean => {
...
const estimatedDiscount = (items: PromotionOrderItem[], promotion: PricePromotion): number => {
    ...
    return promotion.rules.reduce((sum, rule) => {
        ...
        return sum + items.reduce((itemSum, item) => {
            if (rule.productIds?.length && !rule.productIds.includes(item.id)) return itemSum
            if (rule.target === 'product') return itemSum + discountFor(item.basePrice * item.quantity, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
            if (rule.target === 'addon') return itemSum + item.addons.filter((addon) => !rule.addonIds?.length || rule.addonIds.includes(addon.id)).reduce((addonSum, addon) => addonSum + discountFor(addon.amount * addon.priceExtra * item.quantity, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * addon.amount * item.quantity : rule.reward.amount }), 0)
            return itemSum + discountFor(calculateOrderItemTotal(item), { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
        }, 0)
    }, 0)
}

export const calculatePromotionPricing = (items: PromotionOrderItem[], promotions: PricePromotion[], selectedPromotionIds: string[] = []): PromotionPricing => {
    const grossSubtotal = items.reduce((total, item) => total + calculateOrderItemTotal(item), 0)
    const eligible = promotions.filter((promotion) => (promotion.mode === 'automatic' || selectedPromotionIds.includes(promotion.id)) && (!promotion.minSubtotal || grossSubtotal >= promotion.minSubtotal))
    const sorted = [...eligible].sort((a, b) => b.priority - a.priority || estimatedDiscount(items, b) - estimatedDiscount(items, a) || a.id.localeCompare(b.id))
    const accepted: PricePromotion[] = []
    const usedGroups = new Set<string>()
    for (const promotion of sorted) {
        const group = promotion.exclusiveGroup || (promotion.mode === 'automatic' && promotion.rules.some((rule) => rule.target === 'order') ? 'automatic-order' : promotion.combinable ? '' : 'default')
        ...
        for (const promotion of accepted) {
            ...
            items.forEach((item, itemIndex) => {
                ...
                if (rule.target === 'product') {
                    const discountAmount = discountFor(remainingProduct[itemIndex]!, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
                    remainingProduct[itemIndex]! -= discountAmount
                    allocations[itemIndex]!.productDiscountAmount += discountAmount
                }
                if (rule.target === 'line') {
                    const available = remainingProduct[itemIndex]! + remainingAddon[itemIndex]!.reduce((sum, amount) => sum + amount, 0)
                    const discountAmount = discountFor(available, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity : rule.reward.amount })
                    const productDiscountAmount = Math.min(remainingProduct[itemIndex]!, discountAmount)
                    remainingProduct[itemIndex]! -= productDiscountAmount
                    allocations[itemIndex]!.productDiscountAmount += productDiscountAmount
                    let remainingLineDiscount = discountAmount - productDiscountAmount
                    item.addons.forEach((addon, addonIndex) => {
                        const addonDiscount = Math.min(remainingAddon[itemIndex]![addonIndex]!, remainingLineDiscount)
                        remainingAddon[itemIndex]![addonIndex]! -= addonDiscount
                        remainingLineDiscount -= addonDiscount
                        if (addonDiscount) allocations[itemIndex]!.addonDiscounts.push({ addonId: addon.id, discountAmount: addonDiscount })
                    })
                }
                if (rule.target === 'addon') item.addons.forEach((addon, addonIndex) => {
                    if (rule.addonIds?.length && !rule.addonIds.includes(addon.id)) return
                    const discountAmount = discountFor(remainingAddon[itemIndex]![addonIndex]!, { ...rule.reward, amount: rule.reward.type === 'value' ? rule.reward.amount * item.quantity * addon.amount : rule.reward.amount })
                    remainingAddon[itemIndex]![addonIndex]! -= discountAmount
                    if (discountAmount) allocations[itemIndex]!.addonDiscounts.push({ addonId: addon.id, discountAmount })
                })
            })
        }
        }
    ...
    for (const promotion of accepted) {
        ...
        if (rule) {
            ...
            items.forEach((item, itemIndex) => {
                ...
                item.addons.forEach((addon, addonIndex) => {
                    const addonDiscount = Math.min(remainingAddon[itemIndex]![addonIndex]!, remainingDiscount)
                    remainingAddon[itemIndex]![addonIndex]! -= addonDiscount
    ...
    for (const promotion of accepted) {
        ...
        if (discountAmount) appliedPromotions.push({ promotionId: promotion.id, promotionVersion: promotion.version, name: promotion.name, ...(promotion.names ? { names: promotion.names } : {}), mode: promotion.mode, targets: [...new Set(promotion.rules.map((rule) => rule.target))], discountAmount, allocations })
    }
