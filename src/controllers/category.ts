...
const duplicateCategoryError = (error: any) => {
  ...
  return fields
    ? `A category with the same value already exists (${fields})`
...
export const getCategories = async (req: Request, res: Response) => {
  try {
    const categories = await Category.find()
      .sort({ sortOrder: 1, "names.vi": 1, name: 1, _id: 1 })
      .lean();
...
export const createCategory = async (req: Request, res: Response) => {
  try {
    ...
    if (!names)
      return res
        .status(400)
        .json({ success: false, message: "All category names are required" });
    const existing = await Category.findOne({
      $or: [{ "names.vi": names.vi }, { name: names.vi }],
    });
    if (existing) {
      return res
        .status(400)
    ...
    if (error?.code === 11000)
      return res
        .status(409)
        .json({ success: false, message: duplicateCategoryError(error) });
    res
      .status(400)
    ...
    if (updated)
      await emitCatalogEventToStores("catalog.changed", {
        ...
        changedFields: Object.keys(req.body).filter(
          (field) => field === "names" || field === "sortOrder",
        ),
      });
