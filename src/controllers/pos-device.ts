...
function cleanExpiredEnrollment(device: any) {
  return (
    ...
    device.pendingEnrollmentExpiresAt.getTime() <= Date.now()
  );
}
...
export const listPosDevices = async (req: Request, res: Response) => {
  ...
  await Promise.all(
    devices
      .filter(cleanExpiredEnrollment)
      .map((device) =>
        PosDevice.updateOne(
          { _id: device._id },
          {
            $unset: { pendingEnrollmentHash: 1, pendingEnrollmentExpiresAt: 1 },
          },
        ),
      ),
  );
  res.json({
    ...
    data: devices.map((device) => ({
      ...
      hasPendingEnrollment: Boolean(
        device.pendingEnrollmentHash && !cleanExpiredEnrollment(device),
      ),
      pendingEnrollmentExpiresAt: cleanExpiredEnrollment(device)
        ? undefined
        : device.pendingEnrollmentExpiresAt,
    })),
  });
...
export const generateEnrollment = async (req: Request, res: Response) => {
  ...
  if (!device)
    return res
      .status(404)
      .json({ success: false, message: "POS device not found" });
  const enrollment = await createEnrollment(device);
...
export const deleteEnrollment = async (req: Request, res: Response) => {
  const storeId = (req as AuthRequest).user.storeId;
  const result = await PosDevice.updateOne(
...
export const updatePosDevice = async (req: Request, res: Response) => {
  ...
  if (!Object.keys(update).length)
    return res
      .status(400)
      .json({ success: false, message: "No device changes provided" });
      ...
      { _id: String(req.params.id), storeId },
      {
        ...
        ...(req.body.active === false
          ? {
              $unset: {
                deviceTokenHash: 1,
                pendingEnrollmentHash: 1,
                pendingEnrollmentExpiresAt: 1,
              },
            }
          : {}),
      },
...
export const deletePosDevice = async (req: Request, res: Response) => {
  const storeId = (req as AuthRequest).user.storeId;
  const result = await PosDevice.deleteOne({
    _id: String(req.params.id),
    storeId,
  });
...
export const enrollPosDevice = async (req: Request, res: Response) => {
  ...
  if (!code)
    return res
      .status(400)
      .json({ success: false, message: "Enrollment code is required" });
  const device = await PosDevice.findOne({
    pendingEnrollmentHash: hash(code),
    active: true,
  });
  if (
    !device ||
