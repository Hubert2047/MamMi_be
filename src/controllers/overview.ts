...
const parseDateRange = (from: unknown, to: unknown) => {
  ...
  const end = to !== undefined ? zonedBoundary(String(to), true) : new Date();
  if (
...
export const getSuperAdminOverview = async (req: Request, res: Response) => {
  try {
    ...
    if (!range)
      return res
        .status(400)
        .json({ success: false, message: "Invalid overview date range" });
    ...
    const [orders, orderPayments, expenses, revenues, closings] =
      await Promise.all([
        ...
        Expense.aggregate([
          { $match: { storeId: { $in: storeIds }, createdAt: dateFilter } },
          {
            $group: {
              _id: "$storeId",
              expenses: { $sum: "$price" },
        ...
        Revenue.aggregate([
          { $match: { storeId: { $in: storeIds }, createdAt: dateFilter } },
          { $group: { _id: "$storeId", revenue: { $sum: "$price" } } },
        ]),
        DailyClosing.aggregate([
          ...
          { $sort: { confirmedAt: -1 } },
          {
            $group: {
              ...
              lastClosingAt: { $first: "$confirmedAt" },
            },
          },
        ]),
      ]);
    ...
    const data = stores.map((store) => {
      ...
      const orderRevenueByPayment =
        orderPaymentsByStore.get(String(store._id)) ?? {};
      ...
      return {
        ...
        revenue: sales,
        orderRevenue,
        orderRevenueByPayment,
        otherRevenue,
        expenses: cost,
        inventoryExpenses: Number(expense.inventoryExpenses ?? 0),
        otherExpenses: Number(expense.otherExpenses ?? 0),
        profit: sales - cost,
        orders: Number(order.orders ?? 0),
        closingDifference: Number(closing.difference ?? 0),
        closingCount: Number(closing.closings ?? 0),
        lastClosingAt: closing.lastClosingAt ?? null,
      };
    });
    const totals = data.reduce(
      (sum, row) => ({
        ...
        closingDifference: sum.closingDifference + row.closingDifference,
      }),
      { revenue: 0, expenses: 0, profit: 0, orders: 0, closingDifference: 0 },
    );
    return res.json({
      success: true,
      data: { from: range.start, to: range.end, totals, stores: data },
    });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ success: false, message: "Error fetching SuperAdmin overview" });
  }
};
