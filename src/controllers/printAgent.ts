...
async function claimAvailableJob(storeId: string, printerIds: string[], agentId: string) {
    const now = new Date()
    return PrintJob.findOneAndUpdate(
        { storeId, printerId: { $in: printerIds }, $or: [{ status: 'queued' }, { status: 'processing', lockedAt: { $lt: new Date(now.getTime() - retryAfterMs) } }] },
...
export const claimPrintJob = async (req: Request, res: Response) => {
    try {
        const { agentId, storeId, agentDbId } = (req as PrintAgentRequest).printAgent
        const printerIds = (await Printer.find({ storeId, agentId: agentDbId, active: true }).select({ _id: 1 }).lean()).map((printer) => String(printer._id))
        if (!printerIds.length) return res.json({ success: true, data: null })
        const requestedWaitMs = Number(req.query.wait || 0)
        const waitMs = Number.isFinite(requestedWaitMs) ? Math.min(Math.max(requestedWaitMs, 0), 25000) : 0
...
export const completePrintJob = async (req: Request, res: Response) => {
    ...
    const job = await PrintJob.findOneAndUpdate(
        ...
        { $set: { status: 'printed', printedAt: new Date(), retentionUntil: new Date(Date.now() + retentionMs) }, $unset: { lockedAt: 1, lastError: 1 } },
