...
export const createEmployee = async (req: Request, res: Response) => {
    try {
        ...
        const newEmployee = new Employee({ name, numberId, note, active, employmentType, role, salaryType, salaryAmount, startDate, endDate, storeId })
...
export const getEmployees = async (req: Request, res: Response) => {
    try {
        const employees = await Employee.find({ storeId: storeIdFor(req) }).sort({ createdAt: -1 })
...
export const deleteEmployee = async (req: Request, res: Response) => {
    try {
        ...
        const employee = await Employee.findOneAndDelete({ _id: String(id), storeId: storeIdFor(req) } as any)
...
export const updateEmployee = async (req: Request, res: Response) => {
    try {
        ...
        const { name, numberId, note, active, employmentType, role, salaryType, salaryAmount, startDate, endDate, salaryEffectiveFrom, salaryChangeReason } = data
        ...
        if (current.salaryType !== updated.salaryType || current.salaryAmount !== updated.salaryAmount) {
            ...
            await EmployeeSalaryHistory.create({ employeeId: updated._id, salaryType: updated.salaryType, amount: updated.salaryAmount, currency: 'TWD', effectiveFrom, reason: salaryChangeReason })
        }
...
export const getEmployeeSalaryHistory = async (req: Request, res: Response) => {
    try {
        const employee = await Employee.findOne({ _id: String(req.params.id), storeId: storeIdFor(req) } as any).select({ _id: 1 }).lean()
        if (!employee) return res.status(404).json({ success: false, message: 'Employee not found' })
        const history = await EmployeeSalaryHistory.find({ employeeId: employee._id }).sort({ effectiveFrom: -1 }).lean()
        return res.json({ success: true, data: history })
    } catch (error) {
        return res.status(500).json({ success: false, message: 'Error fetching salary history', error })
    }
}
