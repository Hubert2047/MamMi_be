...
export const login = async (
  ...
  try {
    ...
    if (!normalizedAccount || !password) {
      return res
        .status(200)
        .json({ error: false, message: "Account and password are required" });
    }
    ...
    if (!isValid) {
      return res
        .status(200)
        .json({ error: false, message: "Invalid account or password" });
    }
    let activeStoreId =
      user.defaultStoreId?.toString() || user.storeIds?.[0]?.toString();
    if (storeId && !mongoose.isValidObjectId(storeId)) {
      return res.status(400).json({ error: true, message: "Invalid store id" });
    }
    if (storeId) {
      const selectedStore = await Store.findOne({ _id: storeId, active: true })
        .select({ _id: 1 })
        .lean();
      const hasAccess =
        user.role === Role.SuperAdmin ||
        user.storeIds?.some((id) => id.toString() === String(storeId));
      if (!selectedStore || !hasAccess)
        return res
          .status(403)
          .json({
            error: true,
    ...
    if (!activeStoreId) {
      return res
        .status(403)
        .json({ error: true, message: "User is not assigned to a store" });
    }
    const payload = {
...
export const getLoginStores = async (req: Request, res: Response) => {
  try {
    ...
    const normalizedAccount = typeof account === "string" ? account.trim() : "";
    const user = await User.findOne({ account: normalizedAccount })
      .select({ password: 1, role: 1, storeIds: 1 })
      .lean();
    if (
      !user ||
      user.active === false ||
      ...
      return res
        .status(401)
        .json({ error: true, message: "Invalid account or password" });
    const filter =
      user.role === Role.SuperAdmin
        ? { active: true }
        : { _id: { $in: user.storeIds || [] }, active: true };
...
export async function ensureDefaultUsers() {
  ...
  if (!account || !password) {
    throw new Error(
      "DEFAULT_SUPERADMIN_ACCOUNT and DEFAULT_SUPERADMIN_PASSWORD must be configured",
    );
  }
