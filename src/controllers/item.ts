...
export const addStoreItem = async (req: Request, res: Response) => {
    try {
        ...
        const storeItem = await StoreItem.findOneAndUpdate({ storeId, itemId }, { $set: { price }, $setOnInsert: { permanentlyActive: true, temporarilyUnavailable: false, temporarilyUnavailableUntil: null, kitchenPrintEnabled: true, visibility: { pos: true, qr: true, online: true }, addonDisplayMode: 'named' } }, { upsert: true, returnDocument: 'after', includeResultMetadata: false })
...
export const updateStoreItem = async (req: Request, res: Response) => {
    try {
        ...
        if (req.body.kitchenPrintEnabled !== undefined) set.kitchenPrintEnabled = Boolean(req.body.kitchenPrintEnabled)
