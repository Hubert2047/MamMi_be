...
const normalizeNames = (value: unknown) => {
  ...
  return normalized.vi || normalized.en || normalized["zh-TW"]
    ? normalized
    : null;
};
const normalizeOptions = (
  ...
  (Array.isArray(options) ? options : []).map((option: any, index) => {
    if (typeof option === "string") {
      return {
        id: `${prefix}-${index + 1}-${optionLabel(option)}`,
        names: { vi: option, en: option, "zh-TW": option },
      };
    }
    return {
...
const normalizeOptionGroups = (groups: unknown): OptionGroup[] =>
  (Array.isArray(groups) ? groups : [])
    .map((group: any, index) => ({
      ...
      selection: (group?.selection === "multiple" ? "multiple" : "single") as
        | "single"
        | "multiple",
      required: group?.required === true,
      ...(typeof group?.defaultOptionId === "string" &&
      group.defaultOptionId.trim()
        ? { defaultOptionId: group.defaultOptionId.trim() }
        : {}),
  ...
  const ids = [
    ...new Set(
      (Array.isArray(addonIds) ? addonIds : [])
        .map((addon: any) =>
          String(typeof addon === "string" ? addon : addon?.addonId || ""),
        )
        .filter((id) => mongoose.isValidObjectId(id)),
    ),
  ];
  const configByAddonId = new Map(
    (Array.isArray(configs) ? configs : []).map((config: any) => {
      const addonId = String(config?.addonId || "");
      const maxQuantity =
        config?.maxQuantity === null
          ? null
          : Math.max(1, Math.floor(Number(config?.maxQuantity) || 1));
      return [addonId, maxQuantity] as const;
    }),
  );
  return ids.map((addonId) => ({
    addonId,
    maxQuantity: configByAddonId.has(addonId)
      ? configByAddonId.get(addonId)!
      : 1,
  }));
};
...
const toCatalogItem = (item: any, language: string) => ({
  ...
  name:
    ...
    "",
  categoryName:
    item.categoryId?.names?.[language] ||
    item.categoryId?.names?.vi ||
    item.categoryId?.name ||
    "",
  categorySortOrder: Number.isFinite(item.categoryId?.sortOrder)
    ? item.categoryId.sortOrder
    : 0,
  addons: (item.addons || []).map((addon: any) => {
    const config = (item.addonConfigs || []).find(
      (entry: any) => String(entry.addonId) === String(addon._id),
    );
    return {
      ...addon,
      maxQuantity:
        config?.maxQuantity === null ? null : (config?.maxQuantity ?? 1),
    };
  }),
});
...
const clearExpiredTemporaryAvailability = async (storeId: string) => {
  ...
  if (result.modifiedCount > 0)
    emitStoreEvent(storeId, "catalog.store-item.availability.updated", {
      reason: "temporary-availability-expired",
    });
  ...
  if (addonResult.modifiedCount > 0)
    emitStoreEvent(storeId, "catalog.store-addon.availability.updated", {
      reason: "temporary-availability-expired",
    });
};
...
export const getCatalogItems = async (req: Request, res: Response) => {
  try {
    ...
    const items = await Item.find()
      .populate("categoryId", "names name sortOrder")
      .populate("addons", "names name")
      .lean();
    res.json({
      success: true,
      data: items.map((item) => toCatalogItem(item, language)),
    });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Error fetching catalog items", error });
  }
};
  ...
  try {
    ...
    const description =
      req.body.description === undefined
        ? undefined
        : normalizeNames(req.body.description);
    const {
      price: _price,
      active: _active,
      addonConfigs: _addonConfigs,
      ...data
    } = req.body;
    if (data.imageUrl !== undefined && typeof data.imageUrl !== "string")
      return res
        .status(400)
        .json({ success: false, message: "Image URL must be a string" });
    if (
      ...
      return res
        .status(400)
        .json({ success: false, message: "Image public ID must be a string" });
    const type = req.body.type === "combo" ? "combo" : "product";
    const addonConfigs =
      type === "combo"
        ? []
        : normalizeAddonConfigs(req.body.addons, req.body.addonConfigs);
    const item = await Item.create({
      ...data,
      type,
      addons:
        type === "combo" ? [] : addonConfigs.map((config) => config.addonId),
      addonConfigs,
      components:
        type === "combo" ? normalizeComponents(req.body.components) : [],
      names,
      ...(description ? { description } : {}),
      variants: normalizeOptions(req.body.variants, "variant"),
      optionGroups: normalizeOptionGroups(req.body.optionGroups),
      noteOptions: normalizeOptions(req.body.noteOptions, "note"),
    });
    await emitCatalogEventToStores("catalog.item.updated", {
    ...
    res
      .status(400)
      .json({ success: false, message: "Error creating catalog item", error });
  }
};
...
export const updateCatalogItem = async (req: Request, res: Response) => {
  try {
    ...
    const names =
      req.body.names === undefined ? undefined : normalizeNames(req.body.names);
    if (req.body.names !== undefined && !names)
      return res
        .status(400)
        .json({
          success: false,
    ...
    const update = {
      ...
      ...(nextType
        ...
        : {}),
      ...(req.body.type !== "combo" && req.body.components
        ? { components: normalizeComponents(req.body.components) }
        : {}),
      ...(nextType !== "combo" && addonConfigs
        ? { addons: addonConfigs.map((config) => config.addonId), addonConfigs }
        : {}),
      ...(names ? { names } : {}),
      ...(description ? { description } : {}),
      ...(req.body.variants
      ...
      ...(req.body.optionGroups
        ? { optionGroups: normalizeOptionGroups(req.body.optionGroups) }
        : {}),
      ...(req.body.noteOptions
        ? { noteOptions: normalizeOptions(req.body.noteOptions, "note") }
        : {}),
    };
    const previous = await Item.findById(id)
      .select({ imagePublicId: 1 })
      .lean();
    ...
    if (
      ...
      previous.imagePublicId !== item.imagePublicId
    )
      void deleteCloudinaryImage(previous.imagePublicId).catch((error) =>
    ...
    res
      .status(400)
      .json({ success: false, message: "Error updating catalog item", error });
  }
};
...
export const deleteCatalogItem = async (req: Request, res: Response) => {
  try {
    ...
    if (item.imagePublicId)
      void deleteCloudinaryImage(item.imagePublicId).catch((error) =>
    ...
    res
      .status(400)
      .json({ success: false, message: "Error deleting catalog item", error });
  }
};
...
export const addStoreItem = async (req: Request, res: Response) => {
  try {
    ...
    if (!isValidPriceMap(price))
      return res
        .status(400)
        .json({
          success: false,
          code: "INVALID_MONEY_AMOUNT",
          message: "Prices must be non-negative integers",
        });
    ...
    const storeItem = await StoreItem.findOneAndUpdate(
      { storeId, itemId },
      {
        ...
        $setOnInsert: {
          ...
          visibility: { pos: true, qr: true, online: true },
          addonDisplayMode: "named",
        },
      },
      { upsert: true, returnDocument: "after", includeResultMetadata: false },
    );
    emitStoreEvent(storeId, "catalog.store-item.price.updated", {
    ...
    res
      .status(400)
      .json({ success: false, message: "Error adding store item", error });
  }
};
...
export const updateStoreItem = async (req: Request, res: Response) => {
  try {
    ...
    if (
      req.body.permanentlyActive !== undefined &&
      ![Role.Admin, Role.SuperAdmin].includes(authUser.role)
    ) {
      return res
        .status(403)
        .json({
          success: false,
          message: "Only Admin or SuperAdmin can change permanent availability",
        });
    }
    await clearExpiredTemporaryAvailability(storeId);
    const set: Record<string, unknown> = {};
    if (req.body.price !== undefined) {
      if (!isValidPriceMap(req.body.price))
        return res
          .status(400)
          .json({
            success: false,
    ...
    if (req.body.permanentlyActive !== undefined)
      set.permanentlyActive = Boolean(req.body.permanentlyActive);
    if (req.body.visibility && typeof req.body.visibility === "object")
      set.visibility = {
        pos: req.body.visibility.pos !== false,
        qr: req.body.visibility.qr !== false,
        online: req.body.visibility.online !== false,
      };
    if (req.body.addonDisplayMode !== undefined)
      set.addonDisplayMode =
        req.body.addonDisplayMode === "merged" ? "merged" : "named";
    if (req.body.temporarilyUnavailable !== undefined) {
      ...
      if (unavailable)
        set.temporarilyUnavailableUntil = nextStoreMidnight(
          new Date(),
          await getStoreTimeZone(storeId),
        );
    }
    const update: Record<string, unknown> = { $set: set };
    if (req.body.temporarilyUnavailable === false)
      update.$unset = { temporarilyUnavailableUntil: 1 };
    const storeItem = await StoreItem.findOneAndUpdate(
      { storeId, itemId },
      update,
    ...
    if (!storeItem)
      return res
        .status(404)
        .json({ success: false, message: "Item not found in this store" });
    const event =
      Object.prototype.hasOwnProperty.call(set, "price") &&
    ...
    res
      .status(400)
      .json({ success: false, message: "Error updating store item", error });
  }
};
...
export const updateTemporaryStoreItemAvailability = async (
  ...
  try {
    ...
    if (unavailable)
      update.$set = {
        temporarilyUnavailable: true,
        temporarilyUnavailableUntil: nextStoreMidnight(
          new Date(),
          await getStoreTimeZone(storeId),
        ),
      };
    ...
    const storeItem = await StoreItem.findOneAndUpdate(
      { storeId, itemId: String(req.params.itemId) },
      update,
      { returnDocument: "after", includeResultMetadata: false },
    );
    if (!storeItem)
    ...
    emitStoreEvent(storeId, "catalog.store-item.availability.updated", {
      ...
      changedFields: ["temporarilyUnavailable"],
    });
    res.json({ success: true, data: storeItem });
  } catch (error) {
    res
      ...
      .json({
        success: false,
        message: "Error updating temporary availability",
        error,
      });
  }
};
...
export const getItems = async (req: Request, res: Response) => {
  try {
    ...
    const storeItems = await StoreItem.find(storeFilter)
      .populate({
        path: "itemId",
        populate: [
      ...
      .lean();
    const addonIds = storeItems.flatMap(
      (storeItem: any) =>
        storeItem.itemId?.addons?.map((addon: any) => addon._id) || [],
    );
    const storeAddons = await StoreAddon.find({
      storeId,
      addonId: { $in: addonIds },
      permanentlyActive: { $ne: false },
    }).lean();
    const storeAddonById = new Map(
      storeAddons.map((addon: any) => [String(addon.addonId), addon]),
    );

    const result = storeItems
      .filter((storeItem: any) => storeItem.itemId)
      .map((storeItem: any) => {
        ...
        return {
          ...
          visibility: {
            pos: storeItem.visibility?.pos !== false,
            qr: storeItem.visibility?.qr !== false,
            online: storeItem.visibility?.online !== false,
          },
          addonDisplayMode:
            storeItem.addonDisplayMode === "merged" ? "merged" : "named",
          temporarilyUnavailable: storeItem.temporarilyUnavailable === true,
          temporarilyUnavailableUntil:
            storeItem.temporarilyUnavailableUntil || null,
          variants: normalizeOptions(item.variants, "variant"),
          noteOptions: normalizeOptions(item.noteOptions, "note"),
          name:
            item.names?.[language] ||
          ...
          categoryName:
            item.categoryId?.names?.[language] ||
            item.categoryId?.names?.vi ||
            item.categoryId?.names?.en ||
            item.categoryId?.names?.["zh-TW"] ||
            item.categoryId?.name ||
            "",
          addons: item.addons
            ?.filter((addon: any) => storeAddonById.has(String(addon._id)))
            .map((addon: any) => ({
              ...addon,
              maxQuantity: (() => {
                const config = (item.addonConfigs || []).find(
                ...
                return config?.maxQuantity === null
                  ? null
                  : (config?.maxQuantity ?? 1);
              })(),
              priceExtra: storeAddonById.get(String(addon._id)).priceExtra,
              permanentlyActive:
                storeAddonById.get(String(addon._id)).permanentlyActive !==
                false,
              temporarilyUnavailable:
                storeAddonById.get(String(addon._id)).temporarilyUnavailable ===
                true,
              name:
                addon.names?.[language] ||
                addon.names?.vi ||
                addon.names?.en ||
                addon.names?.["zh-TW"] ||
                addon.name ||
...
export const removeSpecificAddons = async () => {
  try {
    const targetIds = [
      "69c5e9a2a98f910697a1956c",
      "69c5e9a3a98f910697a1956e",
    ...
    const result = await Item.updateMany(
      { addons: { $in: targetIds } },
      {
        $pull: {
          addons: { $in: targetIds },
          addonConfigs: { addonId: { $in: targetIds } },
        },
      },
    );
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...item.toObject(),
        price: storeItem.price,
        permanentlyActive: storeItem.permanentlyActive,
        temporarilyUnavailable: storeItem.temporarilyUnavailable,
        temporarilyUnavailableUntil:
          storeItem.temporarilyUnavailableUntil || null,
      },
    });
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching item", error });
  }
};
...
export const createItem = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .json({ success: false, message: "Error creating item", error });
  }
};
...
export const serverUpdateItem = async (
  name: string,
  price: Map<string, number>,
) => {
  try {
  ...
  try {
    ...
    const description =
      req.body.description === undefined
        ...
        : normalizeNames(req.body.description);
    const { price, ...itemData } = req.body;
    if (price !== undefined && !isValidPriceMap(price))
      return res
        .status(400)
        .json({
          success: false,
    ...
    const storeItem = await StoreItem.findOneAndUpdate(
      { storeId, itemId: id },
      { $set: { ...(price !== undefined ? { price } : {}) } },
      { returnDocument: "after", includeResultMetadata: false },
    );
    if (!updated || !storeItem)
      return res
        .status(404)
        .json({ success: false, message: "Item not found in this store" });
    await emitCatalogEventToStores("catalog.item.updated", {
...
export const deleteItem = async (req: Request, res: Response) => {
  try {
    ...
    emitStoreEvent(storeId, "catalog.store-item.availability.updated", {
      itemId: id,
      changedFields: ["permanentlyActive"],
    });
