...
import {
  ...
  calculateStorePromotionPricing,
  getPublicCatalogPromotions,
} from "../services/promotionPricing.js";
import {
...
} from "../utils/publicOrderQuote.js";
...
const onlineOrderingDisabled = () =>
  process.env.ONLINE_ORDERING_ENABLED === "false";
  ...
  if (source === "online" && onlineOrderingDisabled()) {
    res
      ...
      .json({
        ...
        message: "Online ordering is not available yet",
      });
    return true;
  }
  return false;
};
const activeTableForToken = (token: string) =>
  StoreTable.findOne({ qrToken: token, active: true }).lean();
const activeSessionForTable = (storeId: any, tableId: any) =>
...
const activeSessionForId = (storeId: any, sessionId: any) =>
  TableSession.findOne({
    _id: sessionId,
    storeId,
    status: "active",
...
const getQrContext = async (token: string) => {
  ...
  if (!table)
    return { table: null, session: null, code: "QR_NOT_FOUND" as const };
  const session = await activeSessionForTable(table.storeId, table._id);
  if (session) return { table, session, code: null };
  const expired = await TableSession.findOneAndUpdate(
    {
  ...
  return {
    ...
    code: expired
      ...
      : ("SESSION_NOT_ACTIVE" as const),
  };
};

const requireActiveCartSession = async (cart: {
  source?: string;
  storeId: unknown;
  ...
  const reset = await TableSession.findOneAndUpdate(
    {
      ...
      expiresAt: { $gt: now },
      $or: [
        { qrOrderWindowStartedAt: { $lte: cutoff } },
        { qrOrderWindowStartedAt: { $exists: false } },
      ],
    },
    ...
    await TableSession.findOneAndUpdate(
      {
        ...
        status: "active",
        expiresAt: { $gt: now },
        qrOrderWindowStartedAt: { $gt: cutoff },
        qrOrderWindowCount: { $lt: MAX_QR_ORDERS_PER_WINDOW },
      },
...
const mainOnlineStore = () =>
  Store.findOne({ isMain: true, active: true }).select({ name: 1 }).lean();
...
const reserveOnlinePhoneOrderSlot = async (storeId: any, phone: string) => {
  ...
  const windowStartedAt = new Date(
    Math.floor(now.getTime() / ONLINE_PHONE_WINDOW_MS) * ONLINE_PHONE_WINDOW_MS,
  );
    ...
    const limit = await PublicOrderRateLimit.findOneAndUpdate(
      {
        ...
        phoneHash,
        windowStartedAt,
        count: { $lt: MAX_ONLINE_ORDERS_PER_PHONE_WINDOW },
      },
      {
        $inc: { count: 1 },
        $setOnInsert: {
          expiresAt: new Date(
            windowStartedAt.getTime() + ONLINE_PHONE_WINDOW_MS,
          ),
        },
      },
...
const publicRealtimeTokenForStore = (storeId: string) => {
  ...
  return jwt.sign({ scope: "public-catalog", storeId }, realtimeSecret, {
    expiresIn: "15m",
  });
};
...
const verifyTurnstile = async (token: unknown) => {
  ...
  return (
    result.success === true &&
    (!result.action || result.action === "online_order")
  );
};

const validGuestCartLines = (lines: unknown): lines is any[] =>
  Array.isArray(lines) &&
  lines.every(
    (line: any) =>
      line?.itemId &&
      ...
      line.quantity >= 1 &&
      line.quantity <= 99 &&
      Array.isArray(line.optionSelections || []) &&
      Array.isArray(line.noteOptions) &&
...
const buildGuestOrderItems = async (
  storeId: string,
  source: unknown,
  lines: any[],
) => {
  ...
  return lines.map((line: any) => {
    ...
    if (
      line.variant &&
      !item.variants.some((option: any) => option.id === line.variant)
    )
      throw new Error("INVALID_OPTION");
    if (
      line.noteOptions.some(
        (id: string) =>
          !item.noteOptions.some((option: any) => option.id === id),
      )
    )
      throw new Error("INVALID_OPTION");
    const optionSelections = normalizePublicOptionSelections(
    ...
    const addons = line.addonIds.map((id: string) => {
      ...
      return {
        id,
        name: addon.names.vi || addon.names.en || "",
        priceExtra: addon.priceExtra,
        amount: 1,
      };
    });
    return {
      id: line.itemId,
      itemId: randomBytes(12).toString("hex"),
      name: item.names.vi || item.names.en || "",
...
export const getQrMenu = async (req: Request, res: Response) => {
  try {
    const { table, session, code } = await getQrContext(
      String(req.params.token),
    );
    if (!table)
      return res
        .status(404)
        .json({ success: false, code, message: "QR code is not active" });
    if (!session)
      return res
        .status(409)
        .json({
          success: false,
          code,
          message: "Table ordering session is not active",
          table: { code: table.code, name: table.name },
        });
    const store = await Store.findOne({ _id: table.storeId, active: true })
      .select({ name: 1 })
      .lean();
    if (!store)
      return res
    ...
    const [items, promotions] = await Promise.all([
      getPublicMenu(String(table.storeId), "qr"),
      getPublicCatalogPromotions(String(table.storeId)),
    ]);
    ...
    res.json({
      ...
      data: {
        ...
        table: { code: table.code, name: table.name },
        items: applyPublicMenuPromotionDisplays(items, promotions),
        realtimeToken,
      },
    });
  } catch (error) {
...
export const getOnlineMenu = async (_req: Request, res: Response) => {
  try {
    ...
    if (!store)
      return res
        ...
        .json({
          success: false,
          code: "STORE_NOT_AVAILABLE",
          message: "Main store is not available",
        });
    const [items, promotions] = await Promise.all([
      getPublicMenu(String(store._id), "online"),
      getPublicCatalogPromotions(String(store._id)),
    ]);
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to fetch online menu" });
  }
};
  ...
  if (type !== "dine_in" && type !== "takeaway")
    return res
      ...
      .json({
        ...
        message: "Order type must be dine-in or takeaway",
      });
  try {
    const store = await mainOnlineStore();
    if (!store)
    ...
    res
      ...
      .json({
        success: true,
        data: { cartToken: cart.cartToken, type: cart.type, lines: cart.lines },
      });
  } catch {
    res.status(500).json({ success: false, message: "Unable to create cart" });
  }
...
export const getGuestCart = async (req: Request, res: Response) => {
  const cart = await GuestCart.findOne({
    cartToken: String(req.params.cartToken),
  }).lean();
  if (!cart)
    return res
      .status(404)
  ...
  if (!(await requireActiveCartSession(cart)))
    return res
      .status(409)
      .json({
        success: false,
...
export const updateGuestCart = async (req: Request, res: Response) => {
  ...
  if (!lines || !validGuestCartLines(lines))
    return res
      .status(400)
      .json({
        success: false,
        code: "INVALID_CART_LINES",
        message: "Invalid cart lines",
      });
  const existing = await GuestCart.findOne({
    cartToken: String(req.params.cartToken),
    status: "draft",
  })
    .select({ source: 1, storeId: 1, tableSessionId: 1 })
    .lean();
  if (!existing)
    return res
      .status(409)
  ...
  if (!(await requireActiveCartSession(existing)))
    return res
      .status(409)
      .json({
        success: false,
  ...
  if (req.body.type !== undefined) {
    if (req.body.type !== "dine_in" && req.body.type !== "takeaway")
      return res
        .status(400)
        .json({
          success: false,
  ...
  const cart = await GuestCart.findOneAndUpdate(
    { cartToken: String(req.params.cartToken), status: "draft" },
    { $set: update },
...
export const previewGuestCart = async (req: Request, res: Response) => {
  ...
  try {
    const items = await buildGuestOrderItems(
      String(cart.storeId),
      cart.source,
      lines,
    );
    const pricing = await calculateStorePromotionPricing(
      String(cart.storeId),
      items,
    );
    const quote = createPublicOrderQuote(
      cart.cartToken,
      String(cart.storeId),
      lines,
      pricing,
    ...
    res
      ...
      .json({
        success: false,
        code: error?.message || "CART_PREVIEW_FAILED",
        message: "Unable to preview cart",
      });
  }
};

export const confirmGuestCart = async (req: Request, res: Response) => {
  const draft = await GuestCart.findOne({
    cartToken: String(req.params.cartToken),
    status: "draft",
  })
    .select({ source: 1, storeId: 1, tableSessionId: 1 })
  ...
  if (!draft)
    return res
      ...
      .json({
        success: false,
        code: "CART_LOCKED",
        message: "Cart was already confirmed or is unavailable",
      });
  if (rejectDisabledOnlineOrdering(res, draft.source)) return;
  if (!(await requireActiveCartSession(draft)))
    return res
      .status(409)
      .json({
        success: false,
  ...
  try {
    ...
    if (
      ...
      await GuestCart.updateOne(
        { _id: cart._id, status: "confirming" },
        { $set: { status: "draft" } },
      );
      return res
        .status(409)
    ...
    const customer =
      source === "online"
        ? {
            name:
              typeof req.body?.customer?.name === "string"
                ? req.body.customer.name.trim().slice(0, 120)
                : undefined,
            phone:
              normalizePhone(req.body?.customer?.phone).slice(0, 40) ||
              undefined,
            address:
              typeof req.body?.customer?.address === "string"
                ? req.body.customer.address.trim().slice(0, 300)
                : undefined,
          }
        : null;
    if (source === "online") {
      ...
      if (!(await reserveOnlinePhoneOrderSlot(cart.storeId, phone)))
        throw new Error("ONLINE_ORDER_RATE_LIMITED");
    }
    if (
      source === "qr" &&
      !(await reserveQrOrderSlot(cart.storeId, cart.tableSessionId))
    )
      throw new Error("QR_ORDER_RATE_LIMITED");
    const periodId = await getCurrentOrderPeriodId(String(cart.storeId));
    const sequence = await allocateOrderSequence(
      String(cart.storeId),
      periodId,
    );
    const pickupAt =
    ...
    try {
      ...
      console.error("Failed to queue QR kitchen print jobs:", error);
    }
    emitStoreEvent(String(cart.storeId), "order.created", {
    ...
    res
      .status(
        code === "TURNSTILE_NOT_CONFIGURED"
          ? 500
          : code === "ONLINE_ORDER_RATE_LIMITED" ||
              code === "QR_ORDER_RATE_LIMITED"
            ? 429
            : 400,
      )
      .json({ success: false, code, message: "Unable to confirm order" });
  }
};
