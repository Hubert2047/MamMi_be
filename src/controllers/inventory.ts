...
async function validateSupplierConfig(storeId: string, config: SupplierConfig) {
  const supplierIds = Array.isArray(config.supplierIds)
    ...
    : [];
  if (
    Array.isArray(config.supplierIds) &&
    supplierIds.length !== config.supplierIds.length
  )
    throw new Error("Invalid supplier");
  const defaultSupplierId = config.defaultSupplierId
    ? String(config.defaultSupplierId)
    : null;
  if (
    defaultSupplierId &&
    (!mongoose.isValidObjectId(defaultSupplierId) ||
      !supplierIds.includes(defaultSupplierId))
  ) {
    throw new Error("Default supplier must be selected");
  }
  const suppliers = await Supplier.find({ _id: { $in: supplierIds }, storeId })
    .select({ _id: 1 })
    .lean();
...
async function syncIngredientSuppliers(
  storeId: string,
  ...
  if (normalized.supplierIds.length) {
    await IngredientSupplier.insertMany(
      normalized.supplierIds.map((supplierId) => ({
        storeId,
        inventoryItemId,
        supplierId,
...
async function normalizeReceiptLines(
  storeId: string,
  lines: ReceiptInputLine[],
  allowPendingUnitFallback = false,
) {
  ...
  if (items.length !== new Set(itemIds.map(String)).size)
    throw new Error("Invalid inventory item");
  const normalized = lines.map((line) => {
    const item = itemMap.get(String(line.inventoryItemId))!;
    const quantity = Number(line.quantity);
    const unitPrice = Number(line.unitPrice);
    const purchaseUnit = item.purchaseUnits.find(
      (unit) => unit.unitCode === line.unitCode,
    );
    const pendingUnitFallback = allowPendingUnitFallback && !purchaseUnit;
    const resolvedUnitCode = pendingUnitFallback
      ? item.stockUnitCode
      : line.unitCode;
    ...
    return {
      ...
      stockQuantity: quantity * resolvedConversionFactor,
      unitPrice,
...
function stockDeltas(
  lines: Array<{ inventoryItemId: unknown; stockQuantity: number }>,
  multiplier = 1,
) {
  ...
  for (const line of lines)
    deltas.set(
      ...
      (deltas.get(String(line.inventoryItemId)) || 0) +
        line.stockQuantity * multiplier,
    );
  return deltas;
}
...
export const getInventoryItems = async (req: Request, res: Response) => {
  ...
  const items = await InventoryItem.find({ storeId, active: true })
    .sort({ name: 1 })
    .lean();
  const links = await IngredientSupplier.find({
    storeId,
    inventoryItemId: { $in: items.map((item) => item._id) },
  }).lean();
  const linksByItem = new Map<string, typeof links>();
  for (const link of links)
    linksByItem.set(String(link.inventoryItemId), [
      ...(linksByItem.get(String(link.inventoryItemId)) ?? []),
      link,
    ]);
  res.json({
    success: true,
    data: items.map((item) => {
      ...
      return {
        ...item,
        supplierIds: itemLinks.map((link) => String(link.supplierId)),
        defaultSupplierId:
          itemLinks.find((link) => link.isDefault)?.supplierId?.toString() ??
          null,
      };
    }),
  });
...
export const createInventoryItem = async (req: Request, res: Response) => {
  try {
    ...
    if (!name || !stockUnitCode)
      return res
        .status(400)
        .json({ success: false, message: "Name and stock unit are required" });
    const normalizedSuppliers = await validateSupplierConfig(String(storeId), {
      supplierIds,
      defaultSupplierId,
    });
    ...
    await syncIngredientSuppliers(
      String(storeId),
      item._id,
      normalizedSuppliers,
    );
    emitStoreEvent(String(storeId), "inventory.item.updated", {
    ...
    if (error?.code === 11000)
      return res
        .status(409)
        .json({ success: false, message: "Inventory item already exists" });
    return res
      .status(500)
      .json({ success: false, message: "Error creating inventory item" });
  }
};
  ...
  try {
    ...
    if (stockUnitCode && stockUnitCode !== current.stockUnitCode) {
      const [hasReceipt, hasStocktake] = await Promise.all([
        InventoryReceipt.exists({
          storeId,
          inventoryStatus: { $ne: "pending" },
          "lines.inventoryItemId": current._id,
        }),
        InventoryStocktake.exists({
          storeId,
          "lines.inventoryItemId": current._id,
        }),
      ]);
      if (hasReceipt || hasStocktake)
        return res
          .status(400)
          .json({
            success: false,
    ...
    return res
      .status(400)
      .json({ success: false, message: "Invalid inventory item" });
  }
};

export const getInventoryReceipts = async (req: Request, res: Response) => {
  const storeId = (req as AuthRequest).user.storeId;
  const receipts = await InventoryReceipt.find({ storeId })
    .sort({ receivedAt: -1 })
    .populate("lines.inventoryItemId", "name stockUnitCode")
    .lean();
...
export const createInventoryReceipt = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { normalized: normalizedLines, itemMap } =
      await normalizeReceiptLines(String(storeId), lines);
    ...
    const itemNames = [
      ...new Set(
        normalizedLines
          .map((line) => itemMap.get(String(line.inventoryItemId))?.name)
          .filter(Boolean),
      ),
    ];
    const expenseName =
      itemNames.length === 1 ? itemNames[0] : itemNames.join(", ");
    let receipt: any;
    let expense: any;
    await session.withTransaction(async () => {
      const hasPendingItem = [...itemMap.values()].some(
        (item: any) => item.inventoryStatus === "pending",
      );
      receipt = (
          ...
          [
            {
              ...
              totalAmount,
              inventoryStatus: hasPendingItem ? "pending" : "posted",
            },
          ],
          { session },
        )
      ...
      await InventoryReceipt.updateOne(
        { _id: receipt._id },
        { $set: { expenseId: expense._id } },
        { session },
      );
      if (receipt.inventoryStatus !== "pending")
        await applyStockDeltas(
          String(storeId),
          stockDeltas(normalizedLines),
          session,
        );
    });
    ...
    return res
      ...
      .json({
        success: false,
        message: error?.message || "Error creating inventory receipt",
      });
  } finally {
...
export const updateInventoryReceipt = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!receipt)
      return res
        .status(404)
        .json({ success: false, message: "Inventory receipt not found" });
    const { supplierName, receivedAt, note, lines } = req.body;
    const { normalized: normalizedLines, itemMap } =
      await normalizeReceiptLines(storeId, lines);
    const totalAmount = normalizedLines.reduce(
      (sum, line) => sum + line.total,
      0,
    );
    ...
    const deltas =
      receipt.inventoryStatus !== "pending"
        ? stockDeltas(receipt.lines, -1)
        : new Map<string, number>();
    if (receipt.inventoryStatus !== "pending")
      for (const [id, quantity] of stockDeltas(normalizedLines))
        deltas.set(id, (deltas.get(id) || 0) + quantity);
    ...
    const expenseUpdates: Record<string, unknown> = {
      name: itemNames.join(", ") || "Inventory purchase",
      unitPrice: totalAmount,
      price: totalAmount,
    };
    ...
    await session.withTransaction(async () => {
      ...
      if (receipt.expenseId)
        await Expense.updateOne(
          { _id: receipt.expenseId, storeId, type: "inventory_purchase" },
          { $set: expenseUpdates },
          { session },
        );
    });
    ...
    return res
      ...
      .json({
        success: false,
        message: error?.message || "Error updating inventory receipt",
      });
  } finally {
    await session.endSession();
  }
...
export const deleteInventoryReceipt = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      ...
      .json({
        success: false,
        message: error?.message || "Error deleting inventory receipt",
      });
  } finally {
    await session.endSession();
  }
  ...
  try {
    ...
    if (!receipt)
      return res
        .status(404)
        .json({ success: false, message: "Inventory receipt not found" });
    if (receipt.inventoryStatus === "posted")
      return res.json({ success: true, data: receipt });
    const { normalized: lines } = await normalizeReceiptLines(
      storeId,
    ...
    await session.withTransaction(async () => {
      ...
      await InventoryItem.updateMany(
        { _id: { $in: itemIds }, storeId, inventoryStatus: "pending" },
        { $set: { inventoryStatus: "active" } },
        { session },
      );
      await InventoryReceipt.updateOne(
        { _id: receipt._id, storeId },
        {
          $set: {
            lines,
    ...
    return res.json({
      success: true,
      data: { ...receipt, lines, inventoryStatus: "posted" },
    });
  } catch (error: any) {
    return res
      .status(400)
      .json({
        success: false,
        message: error?.message || "Error approving inventory receipt",
      });
  } finally {
    await session.endSession();
  }
