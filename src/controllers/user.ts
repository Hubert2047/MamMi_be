...
function invalidAccount(account: unknown) {
  return (
    ...
    !/^[a-zA-Z0-9._-]{3,50}$/.test(account.trim())
  );
}
...
export const listManagedUsers = async (_req: Request, res: Response) => {
  try {
    const users = await User.find({
      role: { $in: [...managedRoles, Role.SuperAdmin] },
    })
      .select({ password: 0 })
    ...
    users.sort(
      (left, right) =>
        Number(right.role === Role.SuperAdmin) -
          Number(left.role === Role.SuperAdmin) ||
        left.account.localeCompare(right.account),
    );
    return res.json({
      error: false,
      data: users.map((user) => ({ ...user, active: user.active !== false })),
    });
  } catch (error: any) {
...
export const createManagedUser = async (req: Request, res: Response) => {
  try {
    ...
    if (invalidAccount(normalizedAccount))
      return res
        .status(400)
        .json({
          error: true,
          message:
            "Account must be 3-50 characters and use only letters, numbers, dot, underscore or hyphen",
        });
    ...
    if (!managedRoles.includes(role))
      return res
        ...
        .json({
          error: true,
          message: "Only Admin or Employee accounts can be created here",
        });
    if (!mongoose.isValidObjectId(storeId))
      return res
        .status(400)
        .json({ error: true, message: "A valid store is required" });
    ...
    if (!store)
      return res
        .status(400)
        .json({ error: true, message: "Store is not available" });
    if (await User.exists({ account: normalizedAccount }))
      return res
        .status(409)
...
export const updateManagedUser = async (req: Request, res: Response) => {
  try {
    ...
    const normalizedAccount =
      account === undefined
        ...
        : typeof account === "string"
          ? account.trim()
          : "";
    if (normalizedAccount !== undefined && invalidAccount(normalizedAccount))
      return res
        .status(400)
        .json({
          error: true,
    ...
    if (role !== undefined && !managedRoles.includes(role))
      return res
        .status(400)
        .json({
          error: true,
          message: "Only Admin or Employee roles are allowed",
        });
    ...
    if (
      storeId !== undefined &&
      !(await Store.exists({ _id: storeId, active: true }))
    )
      return res
        .status(400)
        .json({ error: true, message: "Store is not available" });
    ...
    if (
      ...
      return res
        ...
        .json({
          error: true,
          message: "The SuperAdmin account can only change its password",
        });
    if (
      normalizedAccount !== undefined &&
      normalizedAccount !== user.account &&
      (await User.exists({ account: normalizedAccount, _id: { $ne: id } }))
    )
      return res
        .status(409)
        .json({ error: true, message: "Account already exists" });
