...
const dailyClosingLineGroupData = async (storeId: string) => {
  const config = await StoreLineGroupConfig.findOne({ storeId })
    .populate({
      path: "dailyClosingLineGroupId",
      select: "lineGroupId name storeId usageStatus",
    })
    .lean();
  const currentId = config?.dailyClosingLineGroupId?._id;
  const groups = await LineGroup.find({
    storeId,
    $or: [
    ...
    .select({ lineGroupId: 1, name: 1, usageStatus: 1 })
    .sort({ name: 1 })
    .lean();
  return {
    enabled: Boolean(currentId),
    lineGroupId: currentId?.toString() ?? null,
    groups,
  };
};
...
export const getDailyClosingLineGroup = async (req: Request, res: Response) => {
  try {
    return res.json({
      success: true,
      data: await dailyClosingLineGroupData((req as AuthRequest).user.storeId),
    });
  } catch (error) {
    console.error(error);
    return res
      ...
      .json({
        success: false,
        message: "Error fetching daily closing LINE group configuration",
      });
  }
};

export const updateDailyClosingLineGroup = async (
  req: Request,
  res: Response,
) => {
  ...
  try {
    ...
    const requestedId =
      enabled && req.body?.lineGroupId ? String(req.body.lineGroupId) : null;
    if (enabled && !requestedId)
      return res
        ...
        .json({
          ...
          message: "A LINE group is required when notifications are enabled",
        });
      ...
      if (result.modifiedCount !== 1)
        return res
          ...
          .json({
            ...
            message: "LINE group is unavailable",
          });
      claimedId = new mongoose.Types.ObjectId(requestedId);
    }
    if (requestedId)
      await StoreLineGroupConfig.findOneAndUpdate(
        { storeId },
        {
          $set: {
            dailyClosingLineGroupId: new mongoose.Types.ObjectId(requestedId),
          },
        },
        { upsert: true },
      );
    else
      await StoreLineGroupConfig.updateOne(
        { storeId },
        { $unset: { dailyClosingLineGroupId: 1 } },
      );
    if (previousId && String(previousId) !== requestedId)
    ...
    return res.json({
      success: true,
      data: await dailyClosingLineGroupData(storeId),
    });
  } catch (error) {
    if (claimedId)
      await LineGroup.updateOne(
    ...
    return res
      ...
      .json({
        success: false,
        message: "Error updating daily closing LINE group configuration",
      });
  }
};
...
export const createDailyClosing = async (req: Request, res: Response) => {
  try {
    const { actualTotal, systemAmount, cash, reason, employeeNumberId } =
      req.body;
    if (
      !Number.isFinite(Number(actualTotal)) ||
      !Number.isFinite(Number(systemAmount)) ||
      !cash ||
      typeof cash !== "object"
    ) {
      return res
        ...
        .json({
          success: false,
          message: "Invalid daily closing amounts or cash data",
        });
    }
    if (!isValidCashData(cash)) {
      return res
        ...
        .json({
          success: false,
          message: "Cash counts must be non-negative integers",
        });
    }
    ...
    if (calculatedActualTotal !== Number(actualTotal)) {
      return res
        ...
        .json({
          success: false,
          message: "Actual total does not match cash counts",
        });
    }
    ...
    if (typeof employeeNumberId !== "string" || !employeeNumberId.trim()) {
      return res
        ...
        .json({
          success: false,
          code: "EMPLOYEE_NUMBER_ID_REQUIRED",
          message: "Employee number ID is required",
        });
    }
    const employee = await Employee.findOne({
      numberId: employeeNumberId.trim(),
      storeId,
    })
      .select({ _id: 1, numberId: 1, name: 1, active: 1 })
      .lean();
    if (!employee) {
      return res
        .status(400)
    ...
    if (
      requiresClosingReason(
        Number(actualTotal) - calculatedSystemAmount,
        reason,
      )
    ) {
      return res
        ...
        .json({
          success: false,
          message: "A reason is required when there is a difference",
        });
    }
    ...
    emitStoreEvent(storeId, "closing.created", {
      closingId: String(dailyClosing._id),
      periodStart: dailyClosing.periodStart,
      periodEnd: dailyClosing.periodEnd,
    });
    ...
    void sendMessageToConfiguredGroups(
      ...
      console.error("Unable to send LINE closing notification", error),
    );
    ...
    if ((error as { code?: number })?.code === 11000) {
      return res
        ...
        .json({
          success: false,
          code: "CLOSING_ALREADY_CREATED",
          message:
            "This closing period was already confirmed by another device",
        });
    }
    console.error(error);
    return res
      .status(500)
      .json({ success: false, message: "Error creating DailyClosing", error });
  }
};
...
export const getDailyClosingSummary = async (req: Request, res: Response) => {
  try {
    const summary = await loadDailyClosingSummary(
      (req as AuthRequest).user.storeId,
    );
    ...
    return res
      ...
      .json({
        success: false,
        message: "Error fetching daily closing summary",
        error,
      });
  }
};
  ...
  try {
    ...
    if (typeof reason !== "string" || !reason.trim()) {
      return res
        ...
        .json({
          success: false,
          message: "A reason is required to void a closing",
        });
    }
    ...
    if (!closing)
      return res
        .status(404)
        .json({ success: false, message: "DailyClosing not found" });
    if (closing.status === "voided")
      return res
        .status(400)
        .json({ success: false, message: "DailyClosing is already voided" });

    const latest = await DailyClosing.findOne({
      storeId,
      status: { $ne: "voided" },
    })
    ...
    if (
      !latest ||
      !canVoidLatestClosing(String(closing._id), String(latest._id))
    ) {
      return res
        .status(409)
        .json({
          success: false,
          message: "Only the latest confirmed closing can be voided",
        });
    }
    ...
    const result = await DailyClosing.updateOne(
      { _id: id, storeId, status: "confirmed" },
      {
        status: "voided",
        voidedAt: new Date(),
        voidedBy: (req as AuthRequest).user.account,
    ...
    if (result.modifiedCount !== 1)
      return res
        ...
        .json({
          success: false,
          message: "DailyClosing changed before it was voided",
        });
    emitStoreEvent(storeId, "closing.voided", { closingId: id });
    return res.json({
      success: true,
      data: await DailyClosing.findOne({ _id: id, storeId }),
    });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ success: false, message: "Error voiding DailyClosing", error });
  }
};
  ...
  try {
    ...
    const [dailyClosings, total, confirmed, voided, latestConfirmed] =
      await Promise.all([
        ...
        DailyClosing.findOne({ storeId: filter.storeId, status: "confirmed" })
          .sort({ periodEnd: -1, createdAt: -1 })
          .select({ _id: 1, periodEnd: 1 })
          .lean(),
      ]);
    return res.json({
      success: true,
      ...
      summary: {
        ...
        confirmed,
        voided,
        latestConfirmedId: latestConfirmed?._id ?? null,
        latestConfirmedPeriodEnd: latestConfirmed?.periodEnd ?? null,
      },
    });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ success: false, message: "Error fetching DailyClosing", error });
  }
};
