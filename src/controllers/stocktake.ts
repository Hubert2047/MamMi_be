...
export const createInventoryStocktake = async (req: Request, res: Response) => {
    try {
        ...
        const stocktake = await InventoryStocktake.create({ storeId, checkedAt: stocktakeDate, lines: normalized })
        await Promise.all(normalized.map((line) => InventoryItem.updateOne({ _id: line.inventoryItemId, storeId }, { $set: { currentQuantity: line.actualQuantity, lastStocktakeAt: stocktakeDate } })))
