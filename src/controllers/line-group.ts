...
export const listLineGroups = async (req: Request, res: Response) => {
  ...
  const filter =
    user.role === Role.SuperAdmin ? {} : { storeId: currentStoreId(req) };
  const groups = await LineGroup.find(filter)
    .select({
      lineGroupId: 1,
      storeId: 1,
      name: 1,
...
export const updateLineGroup = async (req: Request, res: Response) => {
  ...
  if (!req.body || typeof req.body !== "object" || Array.isArray(req.body)) {
    return res
      ...
      .json({
        success: false,
        code: "LINE_GROUP_PAYLOAD_REQUIRED",
        message: "LINE group payload is required",
      });
  }
  const existing = await LineGroup.findOne(
    user.role === Role.SuperAdmin
      ? { _id: id }
      : { _id: id, storeId: currentStoreId(req) },
  );
  if (!existing)
    return res
      .status(404)
      .json({ success: false, message: "LINE group not found" });
  if (existing.usageStatus === "assigned") {
    return res
      ...
      .json({
        ...
        message: "LINE group must be released before editing",
      });
  }
  ...
  if (duplicateName)
    return res
      ...
      .json({
        success: false,
        code: "LINE_GROUP_NAME_EXISTS",
        message: "LINE group name already exists",
      });
  ...
  if (req.body?.storeId !== undefined) {
    ...
    if (storeId && !(await Store.exists({ _id: storeId })))
      return res
        .status(400)
...
export const deleteLineGroup = async (req: Request, res: Response) => {
  ...
  if (!mongoose.isValidObjectId(id))
    return res.status(400).json({ message: "Invalid LINE group id" });
  ...
  if (existing?.usageStatus === "assigned")
    return res
      ...
      .json({
        success: false,
        code: "LINE_GROUP_IN_USE",
        message: "LINE group must be released before deletion",
      });
...
export const testLineGroup = async (req: Request, res: Response) => {
  ...
  const sent = await sendMessageToGroup(
    group.lineGroupId,
    "Đây là tin nhắn kiểm tra từ MamMi. LINE Group đã kết nối thành công.",
  );
  if (!sent)
    return res
      .status(502)
      .json({ message: "Unable to send LINE test message" });
  res.json({ success: true });
};
