...
export const ensureDefaultUnits = async () => {
    await Unit.bulkWrite(expenseUnits.map((unit) => ({
        updateOne: { filter: { code: unit.code }, update: { $setOnInsert: { ...unit, active: true } }, upsert: true },
    })), { ordered: false })
}

export const getUnits = async (_req: Request, res: Response) => {
    const authUser = (_req as AuthRequest).user
    const units = await Unit.find(authUser?.role === Role.SuperAdmin && _req.query.all === 'true' ? {} : { active: true }).sort({ category: 1, code: 1 }).lean()
