...
import {
  ensureStoreAddons,
  ensureStoreCatalog,
  ensureStoreScopedFinancialData,
  migrateStoreAddonAvailability,
  migrateStoreItemAvailability,
} from "./services/storeCatalogMigration.js";
...
(async () => {
  ...
  const promotionExpiryTimer = setInterval(() => {
    void expireEndedPromotions().catch((error) =>
      console.error("Failed to expire promotions:", error),
    );
  }, 60_000);
