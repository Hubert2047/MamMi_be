...
export const deleteCloudinaryImage = async (publicId: string) => {
    ...
    if (!cloudName || !apiKey || !apiSecret || !publicId) throw new Error('Cloudinary is not configured')
    ...
    if (result.result !== 'ok' && result.result !== 'not found') throw new Error(`Cloudinary deletion result: ${result.result || 'unknown'}`)
}
