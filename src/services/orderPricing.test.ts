...
vi.mock('../models/store-item.js', () => ({ default: { find: mocks.storeItemFind } }))
vi.mock('../models/item.js', () => ({ default: { find: mocks.itemFind } }))
vi.mock('../models/store-addon.js', () => ({ default: { find: mocks.storeAddonFind } }))
...
describe('order pricing normalization', () => {
    it('ignores client prices and uses current store item and addon prices', async () => {
        mocks.storeItemFind.mockReturnValue(query([{ itemId: 'item-1', price: { base: 900 }, addonDisplayMode: 'named' }]))
        mocks.itemFind.mockReturnValue(query([{ _id: 'item-1', names: { vi: 'Trà', en: 'Tea', 'zh-TW': '茶' }, variants: [], noteOptions: [], optionGroups: [], addons: [{ _id: 'addon-1', names: { vi: 'Trân châu', en: 'Boba', 'zh-TW': '珍珠' } }], addonConfigs: [] }]))
        mocks.storeAddonFind.mockReturnValue(query([{ addonId: 'addon-1', priceExtra: 300 }]))

        const [item] = await normalizeOrderItemsForPricing('store-1', 'dine_in', [{
            id: 'item-1', itemId: 'line-1', name: 'Forged tea', basePrice: 9999, quantity: 2,
            variant: '', noteOptions: [], note: '', addons: [{ id: 'addon-1', name: 'Forged boba', priceExtra: 9999, amount: 1 }],
        }])

        expect(item?.basePrice).toBe(900)
        expect(item?.addons[0]?.priceExtra).toBe(300)
        expect(item?.name).toBe('Trà')
        expect(item?.printName).toBe('茶')
        expect(item?.printAddons?.[0]?.printName).toBe('珍珠')
    })
