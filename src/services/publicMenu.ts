...
const normalizeOptions = (
  ...
  (Array.isArray(options) ? options : []).map((option: any, index) => {
    ...
    return {
      ...
      names: normalizeNames(option?.names),
    };
  });

const normalizeOptionGroups = (groups: unknown) =>
  (Array.isArray(groups) ? groups : []).map((group: any, index) => {
    const id = group?.id || `option-group-${index + 1}`;
    const options = normalizeOptions(group?.options, `${id}-option`);
    const defaultOptionId = options.some(
...
export async function getPublicMenu(
  ...
  const orderedStoreItems = [...storeItems].sort((left: any, right: any) => {
    ...
    const orderDifference =
      (Number(leftCategory?.sortOrder) || 0) -
      (Number(rightCategory?.sortOrder) || 0);
    ...
    return [
      {
        ...
        category: {
          ...
          names: normalizeNames(
            category?.names || { vi: category?.name || "" },
          ),
          sortOrder: Number.isFinite(category?.sortOrder)
            ? category.sortOrder
            : 0,
        },
        price: basePrice,
        ...
        components: (item.components || []).map(
          (component: any, index: number) => ({
            componentId: `${component.itemId?._id || component.itemId}-${index}`,
            itemId: String(component.itemId?._id || component.itemId),
            quantity: Number(component.quantity) || 1,
            names: normalizeNames(component.itemId?.names),
            noteOptions: normalizeOptions(
              component.itemId?.noteOptions,
              "note",
            ),
          }),
        ),
            ...
            ? [
                {
                  id: String(addon._id),
                  names: normalizeNames(
                    addon.names || { vi: addon.name || "" },
                  ),
                  priceExtra: Number(storeAddon.priceExtra),
                  maxQuantity:
