...
export async function getPublicMenu(storeId: string, channel: 'qr' | 'online' = 'qr') {
    ...
    const storeItems = await StoreItem.find({ storeId, permanentlyActive: true, [`visibility.${channel}`]: { $ne: false } }).populate({
        path: 'itemId',
        populate: [
    ...
    const storeAddons = await StoreAddon.find({ storeId, addonId: { $in: addonIds }, permanentlyActive: true }).lean()
    ...
    return orderedStoreItems.flatMap((storeItem: any) => {
        ...
        return [{
            ...
            price: basePrice,
            unavailable: storeItem.temporarilyUnavailable === true,
            ...
            addons: (item.addons || []).flatMap((addon: any) => {
                ...
                return storeAddon ? [{ id: String(addon._id), names: normalizeNames(addon.names || { vi: addon.name || '' }), priceExtra: Number(storeAddon.priceExtra), maxQuantity: config?.maxQuantity === null ? null : config?.maxQuantity ?? 1, unavailable: storeAddon.temporarilyUnavailable === true }] : []
            }),
        }]
    })
}
