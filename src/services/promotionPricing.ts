...
export const expireEndedPromotions = async (now = new Date()) =>
  Promotion.updateMany(
    { status: "active", endsAt: { $lt: now } },
    { $set: { status: "expired" } },
  );

export const calculateStorePromotionPricing = async (
  storeId: string,
...
export const getPublicCatalogPromotions = async (storeId: string) => {
  ...
  return configs.flatMap((config: any) => {
    ...
    if (
      ...
      !isPromotionAvailableAt(promotion, now)
    )
      return [];
    return [
      {
...
const displayDiscount = (
  ...
  Math.min(
    ...
    Math.max(
      0,
      reward.type === "percent" ? (price * reward.amount) / 100 : reward.amount,
    ),
  );

/** Pre-compute only context-free product/add-on display prices for public menus. */
export const applyPublicMenuPromotionDisplays = (
  items: any[],
  promotions: any[],
) =>
  items.map((item) => {
    const accepted = (matches: (rule: any) => boolean) => {
      ...
      return promotions
        .filter(
          (promotion) =>
            !promotion.minSubtotal && promotion.rules.some(matches),
        )
    ...
    const productMatches = (rule: any) =>
      rule.target === "product" &&
      (!rule.productIds.length || rule.productIds.includes(item.id));
    const displayPrice = accepted(productMatches).reduce(
      (price, promotion) =>
        promotion.rules
          .filter(productMatches)
          .reduce(
            (value: number, rule: any) =>
              value - displayDiscount(value, rule.reward),
            price,
          ),
      item.price,
    );
    return {
      ...
      addons: item.addons.map((addon: any) => {
        const addonMatches = (rule: any) =>
          rule.target === "addon" &&
          (!rule.productIds.length || rule.productIds.includes(item.id)) &&
          (!rule.addonIds.length || rule.addonIds.includes(addon.id));
        const addonDisplayPrice = accepted(addonMatches).reduce(
          (price, promotion) =>
            promotion.rules
              .filter(addonMatches)
              .reduce(
