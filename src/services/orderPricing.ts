...
const printDisplayName = (value: any) => value?.names?.['zh-TW'] || value?.names?.vi || value?.names?.en || value?.name || ''
...
export async function normalizeOrderItemsForPricing(storeId: string, type: OrderType, rawItems: any[]): Promise<PromotionOrderItem[]> {
    ...
    const [storeItems, catalogItems] = await Promise.all([
        StoreItem.find({ storeId, itemId: { $in: itemIds } }).select({ itemId: 1, price: 1, addonDisplayMode: 1, kitchenPrintEnabled: 1, permanentlyActive: 1, temporarilyUnavailable: 1 }).lean(),
    ...
    const printOptionName = (option: any) => typeof option === 'string' ? option : printDisplayName(option) || option?.id || ''
    return rawItems.map((item) => {
        ...
        if (storeItem.permanentlyActive === false) throw new Error('ITEM_NOT_AVAILABLE')
        if (storeItem.temporarilyUnavailable === true) throw new Error('ITEM_TEMPORARILY_UNAVAILABLE')
        ...
        const selectedAddons = (item.addons || []).map((addon: any) => {
            ...
            return { id: String(addon.id), name: displayName(catalogAddon), priceExtra, amount, printName: printDisplayName(catalogAddon) }
        })
        if (new Set(selectedAddons.map((addon: any) => addon.id)).size !== selectedAddons.length) throw new Error('ADDON_QUANTITY_INVALID')
        const printNoteOptions = selectedNoteOptions.map((selectedOption: any) => printOptionName(selectedOption))
        return {
            id: item.id,
            itemId: item.itemId,
            ...
            kitchenPrintEnabled: storeItem.kitchenPrintEnabled !== false,
            noteOptions: printNoteOptions,
            note: item.note,
            printName: printDisplayName(catalogItem),
