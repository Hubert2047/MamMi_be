...
export async function migrateStoreItemAvailability(): Promise<void> {
  await StoreItem.updateMany(
    ...
    [
      {
        $set: {
          permanentlyActive: { $ifNull: ["$active", true] },
          temporarilyUnavailable: false,
          temporarilyUnavailableUntil: null,
        },
      },
  ...
  await StoreItem.updateMany(
    { permanentlyActive: { $exists: false } },
    { $set: { permanentlyActive: true } },
  );
  await StoreItem.updateMany(
    { temporarilyUnavailable: { $exists: false } },
...
export async function ensureStoreScopedFinancialData(
  storeId: string,
): Promise<void> {
  const storeObjectId = new mongoose.Types.ObjectId(storeId);
  await Promise.all([
    Order.updateMany(
      ...
      { $set: { storeId: storeObjectId, source: "pos", version: 1 } },
    ),
    Expense.updateMany(
      { storeId: { $exists: false } },
